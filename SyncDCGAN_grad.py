...
grad_loss = tf.reduce_mean(tf.reduce_sum(tf.square(G1_sample - x1_), axis = 1))
z_gradients = tf.gradients(grad_loss, c1_)
...
saver.restore(sess, "Variable/F.ckpt")
...
for it in range(0):
	...
	z1_batch = np.zeros((batch_size*2, z_dim))
	z2_batch = np.zeros((batch_size*2, z_dim))
...
saver.save(sess, "Variable/F.ckpt")


c = sample_z(batch_size, c_dim)
z = np.zeros((batch_size, z_dim))
x1_sync, x2_sync, s_sync = sync_match_next_batch(x1_train, x2_train, batch_size)
for it in range(1501):
    g = sess.run(z_gradients, feed_dict={z1_:z, c1_:c, x1_:x1_sync})
    g_np = np.asarray(g[0])
    c -= 0.1 * g_np
    if it % 10 == 0:
        print('Iter: {}'.format(it))
        samples = sess.run(G2_sample, feed_dict={z2_:z, c2_:c})
        fig = plot_g(np.concatenate((x1_sync[0:8], samples[0:8]), axis = 0))
