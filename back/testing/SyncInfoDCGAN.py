...
#==================== Discriminator ====================
#Encoder 1
W_m1_e_conv1 = tf.Variable(xavier_init([5,5,1,16]))
b_m1_e_conv1 = tf.Variable(tf.zeros(shape=[16]))

W_m1_e_conv2 = tf.Variable(xavier_init([3,3,16,32]))
b_m1_e_conv2 = tf.Variable(tf.zeros(shape=[32]))

W_m1_e_fc3 = tf.Variable(xavier_init([7*7*32, 256]))
b_m1_e_fc3 = tf.Variable(tf.zeros(shape=[256]))

W_m1_e_fc4_z = tf.Variable(xavier_init([256, z_dim]))
b_m1_e_fc4_z = tf.Variable(tf.zeros(shape=[z_dim]))

W_m1_e_fc4_c = tf.Variable(xavier_init([256, c_dim]))
...
def Encoder1(x):
	...
	h_e_re2 = tf.reshape(h_e_conv2, [-1,7*7*32s])
...
#Ecoder 2
W_m2_e_conv1 = tf.Variable(xavier_init([5,5,1,16]))
b_m2_e_conv1 = tf.Variable(tf.zeros(shape=[16]))

W_m2_e_conv2 = tf.Variable(xavier_init([3,3,16,32]))
b_m2_e_conv2 = tf.Variable(tf.zeros(shape=[32]))

W_m2_e_fc3 = tf.Variable(xavier_init([7*7*32, 256]))
b_m2_e_fc3 = tf.Variable(tf.zeros(shape=[256]))

W_m2_e_fc4_z = tf.Variable(xavier_init([256, z_dim]))
b_m2_e_fc4_z = tf.Variable(tf.zeros(shape=[z_dim]))

W_m2_e_fc4_c = tf.Variable(xavier_init([256, c_dim]))
...
def Encoder2(x):
	...
	h_e_re2 = tf.reshape(h_e_conv2, [-1,7*7*32])
...
#Encoder Loss
cross_ent1 = tf.reduce_mean(-tf.reduce_sum(tf.log(C1_re + 1e-8) * c1_, 1))
ent1 = tf.reduce_mean(-tf.reduce_sum(tf.log(c1_ + 1e-8) * c1_, 1))
Q1_loss = cross_ent1 + ent1
cross_ent2 = tf.reduce_mean(-tf.reduce_sum(tf.log(C2_re + 1e-8) * c2_, 1))
ent2 = tf.reduce_mean(-tf.reduce_sum(tf.log(c2_ + 1e-8) * c2_, 1))
Q2_loss = cross_ent2 + ent2
...
Q1_solver = tf.train.AdamOptimizer().minimize(Q1_loss, var_list=var_e1 + var_g1)
Q2_solver = tf.train.AdamOptimizer().minimize(Q2_loss, var_list=var_e2 + var_g2)
...
for it in range(100001):
	...
	_, loss_q1 = sess.run([Q1_solver, Q1_loss], feed_dict={z1_:z1_batch, c1_:c1_batch})
	_, loss_q2 = sess.run([Q2_solver, Q2_loss], feed_dict={z2_:z2_batch, c2_:c2_batch})
