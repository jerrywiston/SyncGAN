...
#============================== Generator ==============================
# 64 * 64
W_G_conv1 = tf.Variable(xavier_init([3,3,3,32]))
...
# 64 * 64
W_G_deconv5 = tf.Variable(xavier_init([3,3,1,32]))
...
#============================== Discriminator ==============================
# 64 * 64
W_D_conv1 = tf.Variable(xavier_init([5,5,1,64]))
...
def discriminator(X, training):
    ...
    return h_fc_2, tf.nn.sigmoid(h_fc_2)

#============================== Synchronizer ==============================
filter_size = 128
# Mode 1
# 64 * 64
W_m1_conv1 = tf.Variable(xavier_init([5,5,3,filter_size]))
b_m1_conv1 = tf.Variable(tf.zeros(shape=[filter_size]))
# 32 * 32
W_m1_conv2 = tf.Variable(xavier_init([5,5,filter_size,filter_size*2]))
b_m1_conv2 = tf.Variable(tf.zeros(shape=[filter_size*2]))
# 16 * 16
W_m1_conv3 = tf.Variable(xavier_init([5,5,filter_size*2,filter_size*4]))
b_m1_conv3 = tf.Variable(tf.zeros(shape=[filter_size*4]))
# 8 * 8
W_m1_conv4 = tf.Variable(xavier_init([5,5,filter_size*4,filter_size*8]))
b_m1_conv4 = tf.Variable(tf.zeros(shape=[filter_size*8]))
# 4 * 4
W_m1_fc1 = tf.Variable(xavier_init([2*2*filter_size*8,filter_size*4]))
b_m1_fc1 = tf.Variable(tf.zeros(shape=[filter_size*4]))

# Mode 2
# 64 * 64
W_m2_conv1 = tf.Variable(xavier_init([5,5,1,filter_size]))
b_m2_conv1 = tf.Variable(tf.zeros(shape=[filter_size]))
# 32 * 32
W_m2_conv2 = tf.Variable(xavier_init([5,5,filter_size,filter_size*2]))
b_m2_conv2 = tf.Variable(tf.zeros(shape=[filter_size*2]))
# 16 * 16
W_m2_conv3 = tf.Variable(xavier_init([5,5,filter_size*2,filter_size*4]))
b_m2_conv3 = tf.Variable(tf.zeros(shape=[filter_size*4]))
# 8 * 8
W_m2_conv4 = tf.Variable(xavier_init([5,5,filter_size*4,filter_size*8]))
b_m2_conv4 = tf.Variable(tf.zeros(shape=[filter_size*8]))
# 4 * 4
W_m2_fc1 = tf.Variable(xavier_init([2*2*filter_size*8,filter_size*4]))
b_m2_fc1 = tf.Variable(tf.zeros(shape=[filter_size*4]))

# Shared
W_S_fc1 = tf.Variable(xavier_init([filter_size*8,filter_size*4]))
b_S_fc1 = tf.Variable(tf.zeros(shape=[filter_size*4]))

W_S_fc2 = tf.Variable(xavier_init([filter_size*4, 1]))
...
def synchronizer(x1, x2, training):
    ...
    h_conv4_re = tf.reshape(h_conv4, [-1, 2*2*filter_size*8])
    h_conv4_re = tf.layers.batch_normalization(h_conv4_re, training=True)
    ...
    return h_s2, tf.nn.sigmoid(h_s2)
...
#Solver 
G_solver = tf.train.AdamOptimizer(2e-4, beta1=0.5).minimize(G_loss, var_list=var_G)
D_solver = tf.train.AdamOptimizer(1e-3, beta1=0.5).minimize(D_loss, var_list=var_D)

Gs_solver = tf.train.AdamOptimizer(2e-4, beta1=0.5).minimize(Gs_loss, var_list=var_G)
...
x2_train, x1_train = read_data(5000)
