...
#Encoder Loss
E1_loss = tf.reduce_mean(tf.reduce_sum(tf.square(C1_re - c1_), reduction_indices=[1]))
E2_loss = tf.reduce_mean(tf.reduce_sum(tf.square(C2_re - c2_), reduction_indices=[1]))
Gr1_loss = tf.reduce_mean(tf.reduce_sum(tf.square(G1_re - x1_), reduction_indices=[1]))
Gr2_loss = tf.reduce_mean(tf.reduce_sum(tf.square(G2_re - x2_), reduction_indices=[1]))
...
Gr1_solver = tf.train.AdamOptimizer().minimize(Gr1_loss, var_list=var_g1)
Gr2_solver = tf.train.AdamOptimizer().minimize(Gr2_loss, var_list=var_g2)
...
for it in range(20001):
	...
	_, loss_gr1 = sess.run([Gr1_solver, Gr1_loss], feed_dict={x1_:x1_batch})
	_, loss_gr2 = sess.run([Gr2_solver, Gr2_loss], feed_dict={x2_:x2_batch})
	
	_, loss_e1 = sess.run([E1_solver, E1_loss], feed_dict={x1_:x1_batch, z1_:z1_batch, c1_:c1_batch})
	_, loss_e2 = sess.run([E2_solver, E2_loss], feed_dict={x2_:x2_batch, z2_:z2_batch, c2_:c2_batch})
	...
	if it%200 == 0:
		...
		print("  Ss_loss: {:.4}, Gs_loss: {:.4} ".format(loss_ss, loss_gs))
		print()
