...
#Generator
W_g_fc1 = tf.Variable(xavier_init([z_dim,7*7*64]))
b_g_fc1 = tf.Variable(tf.zeros(shape=[7*7*64]))

W_g_conv2 = tf.Variable(xavier_init([5,5,32,64]))
b_g_conv2 = tf.Variable(tf.zeros(shape=[32]))

W_g_conv3 = tf.Variable(xavier_init([5,5,16,32]))
b_g_conv3 = tf.Variable(tf.zeros(shape=[16]))

W_g_conv4 = tf.Variable(xavier_init([5,5,1,16]))
b_g_conv4 = tf.Variable(tf.zeros(shape=[1]))
...
def Generator(z):
    ...
    h_g_conv3 = tf.nn.relu(deconv2d(h_g_conv2, W_g_conv3, output_shape_g3, stride=[1,1,1,1]) + b_g_conv3)
