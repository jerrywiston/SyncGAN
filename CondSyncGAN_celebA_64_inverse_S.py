    ...
    for i in range(size[0]):
        for j in range(size[1]):
            ...
            if i % 2 == 0:
                ...
                plt.imshow(x2_samp[int(i / 2)][j].reshape(64, 64), cmap='Greys_r')
...
def samp_fig(sess, size):
    x1_samp = np.zeros([int(size[0] / 2), size[1], 64, 64, 3], dtype=np.float32)
    x2_samp = np.zeros([int(size[0] / 2), size[1], 64, 64, 1], dtype=np.float32)
...
def generator(X, training):
    ...
    output_shape = tf.stack([tf.shape(X)[0], 64, 64, 1])
...
x1_ = tf.placeholder(tf.float32, shape=[None, 64, 64, 3])
x2_ = tf.placeholder(tf.float32, shape=[None, 64, 64, 1])
